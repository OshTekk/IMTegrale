...
Le calcul repart des notes brutes synchronisées depuis PASS. Les intitulés, semestres, grades et ECTS sont importés depuis le tableau étudiant COMPETENCES avec la même session CAS ; la colonne des crédits tentés/alloués est utilisée afin qu'une UE non validée conserve son vrai coefficient. Le grade COMPETENCES est prioritaire pour le GPA. Lorsqu'il est absent, le grade est calculé depuis la moyenne PASS ; une UE validée après rattrapage reçoit `E`, soit `2,5` points GPA, et reste bien comptée comme validée. Les grades `FX` et `F` signalent au contraire un rattrapage requis, avec `0` ECTS validé. Les notes manuelles et leurs éventuelles modifications ne sont jamais utilisées. Toutes les UE PASS doivent disposer d'un nombre d'ECTS officiel strictement positif avant l'inscription.

À l'activation, seule la dernière génération complète d'ECTS officiels COMPETENCES devient la base des deux scores publics. Les anciennes valeurs manuelles sont ignorées par l'interface et les calculs. Aucune validation manuelle n'est nécessaire et la publication commence immédiatement. Une nouvelle génération COMPETENCES complète renouvelle automatiquement la base ; si elle devient incomplète pour les UE PASS, le profil est retiré sans cooldown et devra consentir de nouveau après une synchronisation valide. L'administrateur peut aussi relancer explicitement cette copie avec un motif. La migration retire tous les anciens profils actifs et révoque leur ancien consentement, tout en conservant le cooldown des utilisateurs qui s'étaient déjà retirés.
...
Le retrait masque immédiatement le profil à tous. Une réactivation est possible sans délai, mais elle publie de nouveau le profil et redémarre intégralement les 48 heures pendant lesquelles l'étudiant ne voit aucun classement, rang ni nombre de participants. L'effacement distinct est lui aussi immédiat : il supprime le consentement et les données propres au leaderboard sans supprimer le compte, l'identité officielle ni les notes. Une suppression complète du compte est également disponible auprès de l'administrateur. Les sauvegardes chiffrées suivent leur cycle normal de rétention ; les données effacées ne réapparaissent pas dans la base active.
