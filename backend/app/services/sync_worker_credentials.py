...
SESSION_PUBLIC = "pass-service-session-public"
...
def self_test_sync_worker_credentials(credentials: SyncWorkerCredentials) -> None:
    try:
        ...
        synthetic_session = secrets.token_bytes(256)
        session_envelope = seal_envelope(
            credentials.service_session.public_key,
            purpose=EnvelopePurpose.PASS_SERVICE_SESSION,
            profile=PlaintextProfile.PASS_SERVICE_SESSION_V1,
            context=session_context,
            plaintext=synthetic_session,
        )
        opened_session = open_envelope(
            session_envelope,
