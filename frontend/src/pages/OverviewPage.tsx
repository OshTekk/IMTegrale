  ...
  return (
    <div className="overview-grid">
      ...
      <section className="content-panel performance-panel">
        ...
        {chart.length ? <div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><BarChart data={chart} margin={{ top: 12, right: 8, left: -24, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--line)" /><XAxis dataKey="name" tickLine={false} axisLine={false} tick={{ fill: "var(--muted)", fontSize: 11 }} interval="preserveStartEnd" minTickGap={16} tickMargin={9} /><YAxis domain={[0, 20]} ticks={[0, 5, 10, 15, 20]} tickLine={false} axisLine={false} tick={{ fill: "var(--soft)", fontSize: 11 }} /><Tooltip cursor={{ fill: "var(--surface-muted)" }} contentStyle={{ background: "var(--surface-raised)", borderRadius: 6, border: "1px solid var(--line-strong)", boxShadow: "var(--shadow)", color: "var(--ink)" }} labelStyle={{ color: "var(--ink)", fontWeight: 800 }} itemStyle={{ color: "var(--brand)" }} formatter={(value) => [`${Number(value).toLocaleString("fr-FR")} /20`, "Moyenne"]} /><ReferenceLine y={10} stroke="#de765e" strokeDasharray="4 4" /><Bar dataKey="moyenne" fill="var(--brand)" radius={[3, 3, 0, 0]} maxBarSize={34} /></BarChart></ResponsiveContainer></div> : <div className="chart-empty"><CircleGauge size={24} /><span>Les moyennes apparaîtront après la première synchronisation.</span></div>}
      </section>
