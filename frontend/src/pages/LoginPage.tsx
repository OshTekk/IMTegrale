      ...
      <div className="login-layout">
        <section className="login-context" aria-label={`Présentation d’${BRAND.name}`}>
          ...
          <div className="security-points">
            ...
            <span>
              <LockKeyhole size={18} /> Session PASS/HUB chiffrée, 30 jours maximum
            </span>
          </div>
          <SourceNotice />
          <div className="login-public-links">
            <Link to="/confiance">
              <CircleHelp size={17} />
              <span>
                <strong>Pourquoi puis-je me connecter ?</strong>
                <small>Comprendre les données, le chiffrement et ses limites</small>
              </span>
            </Link>
            <Link to="/demo">
              <PlayCircle size={17} />
              <span>
                ...
                <small>Suivre un parcours fictif, étape par étape</small>
              </span>
            </Link>
            <a className="login-source-link" href={BRAND.sourceCodeUrl} target="_blank" rel="noreferrer">
              <GitHubMark size={17} />
              <span>
                <strong>Code source public</strong>
                <small>Examiner l'architecture, les protections et la documentation</small>
              </span>
              <ExternalLink size={13} />
            </a>
          </div>
          <p className="login-paff-credit">
            Avec un clin d'œil à{" "}
            <a href={BRAND.paffUrl} target="_blank" rel="noreferrer">
              PAFF de Lucien Hervé <ExternalLink size={12} />
            </a>
        ...
        <section className="login-panel">
          ...
          <div className="segmented login-methods" role="tablist" aria-label="Méthode de connexion">
            <button
              className={mode === "imt" ? "active" : ""}
              onClick={() => selectMode("imt")}
              type="button"
              role="tab"
              aria-selected={mode === "imt"}
            >
              <School size={17} /> Compte IMT
            </button>
            {passkeysSupported() && (
              <button
                className={mode === "passkey" ? "active" : ""}
                onClick={() => selectMode("passkey")}
                type="button"
                role="tab"
          ...
          <div className={`login-method-note ${mode}`}>
            ...
            <p>
              {mode === "imt" ? (
                <>
                  <strong>Nouveau sur IMTégrale ? Commence ici.</strong> Ton espace est créé après vérification par
                  l'IMT. Si ton compte existe déjà, aucune synchronisation de notes n'est lancée.
                </>
              ) : mode === "passkey" ? (
                <>
                  <strong>Accès rapide recommandé après l'inscription.</strong> Utilise la clé enregistrée sur cet
                  appareil, sans contacter PASS.
                </>
              ) : (
                <>
                  <strong>Accès personnel ou partagé.</strong> Utilise un token généré depuis un compte IMTégrale
                  existant.
                </>
              )}
            </p>
          ...
          <form onSubmit={submit} className="login-form">
            {mode === "passkey" && (
              <div className="passkey-login-choice">
                <span>
                  <Fingerprint size={28} />
                </span>
                <div>
                  <strong>Connexion sans mot de passe</strong>
                  <p>Utilise Face ID, Touch ID, Windows Hello ou la sécurité de ton appareil.</p>
                </div>
              </div>
            )}
            {mode === "imt" && (
              <>
                ...
                <p className="field-note">
                  Le mot de passe transite uniquement pour l'authentification CAS et n'est jamais enregistré. La
                  première connexion importe ton espace ; les suivantes ne synchronisent rien sans action distincte.
                </p>
              </>
            )}
            {mode === "token" && (
              <>
                ...
                <p className="field-note">
                  Les droits et l'expiration dépendent du token. Il peut être révoqué à tout moment.
                </p>
              </>
            )}
            {login.error && (
