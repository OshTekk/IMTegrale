...
const steps: DemoStep[] = [
  { title: "Tu demandes ton inscription", short: "Inscription", detail: "Le navigateur envoie une fois l'identifiant CAS et le mot de passe IMT sur une connexion HTTPS valide. Les protections par compte et par connexion cliente s'appliquent avant tout appel à PASS.", proof: "Le mot de passe sert uniquement à cette authentification et n'est jamais écrit en base.", node: "browser", icon: CircleUserRound },
  ...
  { title: "La session technique est isolée", short: "Session privée", detail: "Après l'authentification, le mot de passe est détruit. Seuls les cookies nécessaires à PASS et au Hub sont filtrés, chiffrés en AES-256-GCM et conservés au maximum 30 jours.", proof: "La base ne contient ni mot de passe IMT ni cookie lisible ; la session peut être révoquée et ne survit pas au-delà de sa limite locale.", node: "vault", icon: LockKeyhole },
  ...
  { title: "Moyenne et GPA sont calculés", short: "Calculs", detail: "Les notes construisent une moyenne par UE. La moyenne générale et le GPA sont ensuite pondérés par les crédits ECTS de chaque UE ; un rattrapage validé correspond au grade E et à 2,5.", proof: "Deux résultats distincts, un même poids académique : les ECTS. Toute nouvelle donnée importée est poussée vers l'interface en direct.", node: "dashboard", icon: Calculator },
  { title: "Chaque semestre reste lisible", short: "Notes & ECTS", detail: "Les vues Notes et UE & ECTS détaillent les évaluations officielles, les coefficients, les grades et les crédits. Les filtres S5 à S10 permettent de retrouver les résultats et le GPA de chaque semestre.", proof: "Une donnée PASS ou COMPETENCES ne peut être ni ajoutée, ni modifiée, ni masquée par l'étudiant.", node: "dashboard", icon: TableProperties },
  { title: "Les actualisations restent sous contrôle", short: "Synchronisation", detail: "La reconnexion ne synchronise rien. Le manuel est limité et l'automatique est désactivé par défaut, limité aux heures ouvrées et au minimum à deux heures d'intervalle.", proof: "La session PASS est réutilisée tant qu'elle reste valide. À son expiration, toute automatisation se met en pause jusqu'à une reconnexion volontaire.", node: "dashboard", icon: Clock3 },
  { title: "Tu choisis tes moyens d'accès", short: "Accès & partage", detail: "Une passkey ouvre le compte sans mot de passe ni appel à PASS. Un token personnel peut servir d'alternative, tandis qu'un token de partage reçoit uniquement le rôle et l'expiration choisis.", proof: "Le secret d'un token n'est affiché qu'une fois, n'est conservé que sous forme d'empreinte et reste révocable.", node: "optional", icon: Share2, optional: true },
  ...
  { title: "Les hypothèses restent séparées du réel", short: "Simulations", detail: "Chaque étudiant peut conserver cinq simulations GPA et cinq simulations de notes. Il peut importer son état académique, modifier la copie, ajouter de futures UE et comparer des scénarios avec autosauvegarde.", proof: "Aucune hypothèse n'altère une donnée officielle, une notification ou le classement.", node: "optional", icon: FlaskConical, optional: true },
  { title: "Les calendriers rejoignent le même espace", short: "Agenda", detail: "Un lien iCalendar INPASS alimente l'agenda personnel avec une actualisation horaire. Les étudiants FIP peuvent aussi consulter les calendriers de formation 2027, 2028 et 2029.", proof: "Le lien INPASS est chiffré, jamais réaffiché et absent des accès partagés.", node: "optional", icon: CalendarDays, optional: true },
  { title: "Un relevé personnel peut être généré", short: "Relevé PDF", detail: "Le relevé rassemble identité, UE, notes, grades, ECTS et calculs, avec un filtre par semestre et une annexe facultative. Il indique clairement qu'il s'agit d'un document informatif non officiel.", proof: "Le PDF est généré à la demande, n'est jamais stocké sur le serveur et renvoie vers les sources et le code du calcul.", node: "optional", icon: FileDown, optional: true },
  ...
  return (
    <div className="demo-page">
      <section className="public-title-band demo-title-band">
        ...
        <div><span className="section-kicker">Données entièrement fictives</span><h1>Explorer tout le parcours IMTégrale</h1><p>Sélectionne une étape pour voir ce qui circule, ce qui reste privé et comment chaque fonction se comporte.</p></div>
      ...
      <section className="demo-summary-band"><div><LockKeyhole size={20} /><span><strong>Privé par défaut</strong>Notes, identité, agenda, relevés et simulations restent liés au compte.</span></div><div><ShieldCheck size={20} /><span><strong>Choix séparés</strong>Automatisation, Telegram, partage et classement s'activent indépendamment.</span></div><Link className="primary-button" to="/">Accéder à la connexion</Link></section>
    </div>
  );
}
