...
export function NotesPage() {
  ...
  const yearByUe = useMemo(
    () => new Map(dashboard.data?.ues.map((ue) => [ue.code, ue.year]) ?? []),
    [dashboard.data?.ues],
  );
  const semesterByUe = useMemo(
    () => new Map(dashboard.data?.ues.map((ue) => [ue.code, ue.semester]) ?? []),
    [dashboard.data?.ues],
  );
  const notes = useMemo(
    () =>
      (dashboard.data?.notes ?? []).filter((note) => {
      ...
      <section className="toolbar-band">
        <div className="search-field">
          ...
          <input
            ...
            placeholder="Rechercher une UE ou une évaluation"
      ...
      <section className="data-section">
        <header className="section-heading">
          <div>
            ...
            <p>
              {notes.length} résultat{notes.length > 1 ? "s" : ""} officiel{notes.length > 1 ? "s" : ""}
            </p>
          </div>
          <span className="official-data-label">
            <BadgeCheck size={16} /> Source PASS
          </span>
        </header>
        {notes.length ? (
            ...
            <table className="data-table notes-table">
              ...
              <tbody>
                {notes.map((note) => (
                  <tr key={note.id}>
                    <td>
                      <strong className="code-label">{note.ue_code}</strong>
                    </td>
                    <td>
                      <div className="cell-primary">
                        <strong>{note.label}</strong>
                        <span>{note.is_resit ? "Rattrapage" : "Évaluation classique"}</span>
                      </div>
                    </td>
                    <td data-label="Note">
                      <strong className={note.score >= 10 ? "score success" : "score danger"}>
                        {formatNumber(note.score)}
                        <small>/20</small>
                      </strong>
                    </td>
                    <td data-label="Coeff.">{formatNumber(note.coefficient)}</td>
                    <td data-label="Source">
                      <span className="source-badge pass">PASS</span>
                    </td>
                    <td data-label="Détection">
                      <span className="date-cell">{formatDate(note.detected_at, false)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <EmptyState title="Aucune note trouvée" detail="Aucune note PASS ne correspond aux filtres sélectionnés." />
        )}
      </section>
    </div>
  );
