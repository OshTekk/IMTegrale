...
import { Filter, Pencil, Plus, RotateCcw, Search, Trash2, TriangleAlert } from "lucide-react";
...
function NoteEditor({ note, onClose }: { note: NoteItem | null; onClose: () => void }) {
  ...
  const remove = useMutation({
    ...
    onSuccess: () => { refresh(); showToast(note?.source === "pass" ? "Note masquée" : "Note supprimée"); onClose(); },
    onError: (error) => showToast(error.message, "error")
  });
  const reset = useMutation({
    mutationFn: () => api(`/api/v1/notes/${note?.id}`, { method: "PATCH", body: JSON.stringify({ clear_overrides: true }) }),
    onSuccess: () => { refresh(); showToast("Valeurs PASS restaurées"); onClose(); },
    onError: (error) => showToast(error.message, "error")
  });
  const submit = (event: FormEvent) => { event.preventDefault(); save.mutate(); };

  return (
    <Modal open title={note ? "Modifier la note" : "Ajouter une note"} description={note?.source === "pass" ? "Les modifications restent locales et ne changent pas PASS." : undefined} onClose={onClose}>
      <form className="modal-form" onSubmit={submit}>
        <div className="form-grid two-columns">
          <label>UE<input value={values.ue_code} disabled={note?.source === "pass"} onChange={(event) => setValues({ ...values, ue_code: event.target.value.toUpperCase() })} placeholder="SIT130" required /></label>
        ...
        <footer className="modal-actions split-actions">
          <div>{note?.has_override && <button className="text-button" type="button" onClick={() => reset.mutate()} disabled={reset.isPending}><RotateCcw size={16} /> Restaurer PASS</button>}{note && <button className={deleteArmed ? "danger-button armed" : "danger-button"} type="button" onClick={() => deleteArmed ? remove.mutate() : setDeleteArmed(true)} disabled={remove.isPending}><Trash2 size={16} /> {deleteArmed ? "Confirmer" : note.source === "pass" ? "Masquer" : "Supprimer"}</button>}</div>
...
export function NotesPage({ role }: { role: Role }) {
  ...
  }), [dashboard.data?.notes, search, source, year, yearByUe]);
  ...
  return (
    <div className="page-stack">
      <section className="toolbar-band">
        ...
        <div className="filter-group"><Filter size={17} /><select value={year} onChange={(event) => setYear(event.target.value)} aria-label="Filtrer par année"><option value="all">Toutes les années</option>{years.map((item) => <option key={item} value={item}>{yearLabel(item)}</option>)}</select><select value={source} onChange={(event) => setSource(event.target.value)} aria-label="Filtrer par source"><option value="all">Toutes les sources</option><option value="pass">PASS</option><option value="manual">Manuelles</option></select></div>
