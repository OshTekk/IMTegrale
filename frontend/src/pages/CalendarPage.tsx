...
import type { CalendarEventItem, CalendarStatus, FipTrainingCalendar, FipTrainingPeriod } from "../types";
...
function eventDateLabel(event: CalendarEventItem): string {
  if (event.all_day) {
    ...
    return event.start === finalDay
      ? `${start} · toute la journée`
      : `${start} – ${formatPlainDate(finalDay, { weekday: "long", day: "numeric", month: "long", year: "numeric" })}`;
  }
    ...
    >
      <form className="modal-form calendar-connect-form" onSubmit={submit}>
        ...
        <details className="calendar-link-guide">
          <summary>
            <Info size={17} /> Où trouver ce lien ?
          </summary>
          <ol>
        ...
        {connect.isError && (
          <p className="form-error">
            <CircleAlert size={16} /> {calendarErrorMessage(connect.error)}
          </p>
        )}
...
function PersonalCalendar({ status }: { status: CalendarStatus }) {
  ...
  const events = useCalendarEvents(visibleRange?.start ?? null, visibleRange?.end ?? null, status.configured);
    ...
    return (
      <section className="calendar-onboarding data-section">
        <header className="section-heading">
          <div>
            <span className="section-kicker">Agenda personnel</span>
            <h2>Retrouve tes cours dans IMTégrale</h2>
          </div>
          <span className="official-data-label">
            <ShieldCheck size={16} /> Privé
          </span>
        </header>
        <div className="calendar-onboarding-body">
          <EmptyState
            ...
            action={
              <button className="primary-button" type="button" onClick={() => setConnectionOpen(true)}>
                <Link2 size={17} /> Ajouter mon lien
              </button>
            }
          />
          <dl className="calendar-privacy-facts">
            <div>
              <dt>
                <LockKeyhole size={18} /> Lien protégé
              </dt>
              <dd>Chiffré et jamais affiché après l'enregistrement.</dd>
            </div>
            <div>
              <dt>
                <RefreshCw size={18} /> Toutes les heures
              </dt>
              <dd>Une seule récupération planifiée par compte.</dd>
            </div>
            <div>
              <dt>
                <Trash2 size={18} /> Révocable
              </dt>
              <dd>Le lien et tous les cours importés sont supprimables immédiatement.</dd>
            </div>
          </dl>
        </div>
  ...
  return (
    <div className="calendar-personal-stack">
          ...
          <div>
            ...
            <p>
              {status.last_status === "error"
                ? syncErrorMessage(status.last_error_code)
                : `${status.event_count} cours importés · ${status.account_hint ?? "compte IMT"}`}
            </p>
          </div>
        </div>
        <dl>
          <div>
            ...
            <dd>{status.last_success_at ? relativeDate(status.last_success_at) : "En attente"}</dd>
          </div>
          <div>
            <dt>Prochaine vérification</dt>
            <dd>{status.next_refresh_at ? relativeDate(status.next_refresh_at) : "Dans l'heure"}</dd>
          </div>
        </dl>
        <button className="secondary-button" type="button" onClick={() => setConnectionOpen(true)}>
          <Settings2 size={17} /> Gérer
        </button>
      </section>
      ...
      <section className="personal-calendar data-section" aria-busy={events.isFetching}>
        ...
        {!events.isFetching && !events.isError && events.data?.length === 0 && status.event_count > 0 && (
          <div className="calendar-range-empty">
            <Info size={16} /> Aucun cours importé sur cette période.
          </div>
        )}
      ...
      <Modal
        ...
        description="Cette action retire le lien privé et tous les cours importés."
      ...
      >
        <div className="calendar-disconnect-dialog">
          ...
          <div className="modal-actions">
            <button className="secondary-button" type="button" onClick={() => setDisconnectOpen(false)}>
              Annuler
            </button>
            <button className="danger-button" type="button" onClick={removeCalendar} disabled={disconnect.isPending}>
              {disconnect.isPending ? <LoaderCircle className="spin" size={17} /> : <Trash2 size={17} />} Supprimer
            </button>
          </div>
        </div>
      </Modal>
      <Modal
        ...
        title={selectedEvent?.title ?? "Cours"}
        description="Détail importé depuis INPASS"
          ...
          <div className="calendar-event-detail">
            <div>
              ...
              <span>
                ...
                <p>{eventDateLabel(selectedEvent)}</p>
              </span>
            </div>
            <div>
              <MapPin size={19} />
              <span>
                <strong>Lieu</strong>
                <p>{selectedEvent.location || "Non indiqué dans INPASS"}</p>
              </span>
            </div>
          </div>
        )}
      ...
      <div className="calendar-data-controls">
        <span>
          <LockKeyhole size={16} /> Cet agenda n'est jamais accessible depuis un token de partage.
        </span>
        <button type="button" onClick={() => setDisconnectOpen(true)}>
          <Trash2 size={15} /> Supprimer mon agenda
        </button>
      </div>
    </div>
  );
...
function TrainingCalendarView({ calendar }: { calendar: FipTrainingCalendar }) {
  ...
  if (!promotion)
    return (
      <EmptyState title="Calendrier indisponible" detail="Aucune promotion ne figure dans le calendrier source." />
    );
    ...
    <div className="training-calendar-stack">
      <section className="training-heading-band">
        ...
        <div className="training-current-state">
          ...
          <div>
            <small>{activePeriod ? "Période actuelle" : "Prochaine période"}</small>
            <strong>
              {activePeriod
                ? activePeriod.kind === "school"
                  ? "Formation"
                  : "Entreprise"
                : nextPeriod
                  ? `${nextPeriod.kind === "school" ? "Formation" : "Entreprise"} · ${formatPlainDate(nextPeriod.start, { day: "numeric", month: "long" })}`
                  : "Année terminée"}
            </strong>
          </div>
        </div>
      ...
      <section className="training-promotion-selector" aria-label="Choisir une promotion FIP">
        <div className="segmented" role="tablist" aria-label="Promotion">
          {calendar.promotions.map((item) => (
            ...
            >
              <span>FIP {item.promotion_year}</span>
              <small>{item.level}</small>
            </button>
          ))}
        </div>
        {calendar.default_promotion_year === promotion.promotion_year && (
          <span className="current-promotion-label">
            <BadgeCheck size={15} /> Ta promotion
          </span>
        )}
      </section>
      ...
      <section className="training-timeline data-section">
        <header className="section-heading">
          <div>
            <span className="section-kicker">Vue annuelle</span>
            <h2>
              FIP {promotion.promotion_year} · {promotion.level}
            </h2>
          </div>
          <div className="training-legend">
            ...
            {promotion.milestones.length > 0 && (
              <span className="mobility">
                <i /> International
              </span>
            )}
          </div>
        ...
        <div className="training-timeline-chart">
          <div className="training-month-axis" aria-hidden="true">
            {markers.map((marker) => (
              <span key={`${marker.label}-${marker.left}`} style={{ left: `${marker.left}%` }}>
                {marker.label}
              </span>
            ))}
          </div>
          <div className="training-track" aria-label="Périodes de formation et d'entreprise">
            {promotion.periods.map((period) => (
              <span
                ...
                className={`training-block ${period.kind}`}
                style={toTimelineStyle(period.start, period.end, rangeStart, rangeEnd)}
                title={`${period.kind === "school" ? "Formation" : "Entreprise"} · ${formatPlainRange(period.start, period.end)}`}
              >
                <i>{period.weeks} sem.</i>
                {period.campus && <b>{period.campus}</b>}
              </span>
            ))}
          </div>
          <div className="semester-track" aria-label="Semestres">
            {promotion.semesters.map((semester) => (
              <span key={semester.semester} style={toTimelineStyle(semester.start, semester.end, rangeStart, rangeEnd)}>
                <strong>{semester.semester}</strong>
                <small>{formatPlainRange(semester.start, semester.end)}</small>
              </span>
            ))}
          </div>
          {promotion.milestones.length > 0 && (
            <div className="milestone-track" aria-label="Périodes internationales">
              {promotion.milestones.map((milestone) => (
                <span
                  key={milestone.kind}
                  className={milestone.kind}
                  style={toTimelineStyle(milestone.start, milestone.end, rangeStart, rangeEnd)}
                >
                  <ExternalLink size={14} /> {milestone.kind === "international_project" ? "PSI" : "Mobilité"}
                </span>
              ))}
            </div>
          )}
      ...
      <section className="training-details data-section">
        <header className="section-heading">
          <div>
            <span className="section-kicker">Périodes</span>
            <h2>Dates de la promotion {promotion.promotion_year}</h2>
          </div>
          <div className="training-totals">
            <span>
              <GraduationCap size={16} /> {promotion.totals.school_weeks} sem.
            </span>
            <span>
              <Building2 size={16} /> {promotion.totals.company_weeks} sem.
            </span>
          </div>
        </header>
        <div className="training-period-list">
          {promotion.periods.map((period) => (
            <TrainingPeriodRow key={`${period.kind}-${period.start}`} period={period} />
          ))}
        </div>
        {promotion.milestones.length > 0 && (
          <div className="training-milestones">
            {promotion.milestones.map((milestone) => (
              <div key={milestone.kind}>
                <span>
                ...
                <div>
                  ...
                  <p>
                    {formatPlainRange(milestone.start, milestone.end)} · {milestone.detail}
                  </p>
                </div>
              </div>
            ))}
      ...
      <footer className="training-source-note">
        <Info size={16} />
        <span>
          <strong>{calendar.source.label}</strong>Version du{" "}
          {formatPlainDate(calendar.source.version_date, { day: "numeric", month: "long", year: "numeric" })}.{" "}
          {calendar.campus_note}
        </span>
      </footer>
    </div>
  );
...
function TrainingPeriodRow({ period }: { period: FipTrainingPeriod }) {
  ...
  return (
    <div className="training-period-row">
      <span className={school ? "school" : "company"}>
        {school ? <GraduationCap size={18} /> : <Building2 size={18} />}
      </span>
      <div>
        <strong>{school ? "Formation" : "Entreprise"}</strong>
        <p>{formatPlainRange(period.start, period.end)}</p>
      </div>
      <span className="training-week-count">{period.weeks} semaines</span>
      {period.campus && (
        <span className="campus-badge">
          <MapPin size={14} /> {period.campus}
        </span>
      )}
    </div>
  );
...
export function CalendarPage() {
  ...
  const training = useFipTrainingCalendar(status.data?.fip_training_available === true && section === "training");
  ...
  if (status.isPending)
    return (
      <div className="calendar-page-skeleton">
  ...
  if (status.isError || !status.data)
    return (
      <div className="error-panel">
        <CircleAlert size={22} /> {status.error?.message ?? "L'agenda est indisponible."}
      </div>
    );
    ...
    <div className="page-stack calendar-page">
      {status.data.fip_training_available && (
        <section className="calendar-section-tabs">
          <div className="segmented" role="tablist" aria-label="Type de calendrier">
            ...
            >
              <CalendarDays size={17} /> Mes cours
            </button>
            <button
              type="button"
              role="tab"
            ...
            >
              <GraduationCap size={17} /> Formation FIP
            </button>
          </div>
          <span>
            <ShieldCheck size={16} /> L'agenda de cours reste strictement personnel.
          </span>
        </section>
      )}
      ...
      {section === "training" &&
        ...
        (training.isPending ? (
          ...
          <div className="error-panel">
            <CircleAlert size={22} /> {training.error?.message ?? "Le calendrier de formation est indisponible."}
          </div>
        ) : (
