...
export function SettingsPage({ role }: { role: Role }) {
  ...
  if (role !== "owner") return <div className="settings-grid"><section className="settings-panel access-only"><span className="large-status-icon"><ShieldCheck size={26} /></span><h2>Accès en lecture seule</h2><p>Seul le propriétaire peut modifier le compte, Telegram et les accès.</p><div className="info-line"><KeyRound size={17} /><span>Méthode</span><strong>Token d'accès</strong></div></section></div>;
  ...
  return (
    <div className="settings-grid">
      ...
      <section className="settings-panel sync-settings">
        ...
        {data.sync.paused_reason === "reauth_required" && <div className="inline-warning"><TriangleAlert size={16} /> L'actualisation automatique est en pause jusqu'au renouvellement de la session IMT.</div>}
        <div className={`pass-session-summary state-${data.sync.service_session.state}`}>
          <span><LockKeyhole size={18} /></span>
          <div><strong>{data.sync.service_session.state === "active" ? "Session PASS active" : data.sync.service_session.state === "owner_managed" ? "Synchronisation propriétaire autonome" : "Reconnexion IMT requise"}</strong><small>{data.sync.service_session.state === "active" ? `Dernière utilisation ${formatDate(data.sync.service_session.last_used_at)} · limite locale ${formatDate(data.sync.service_session.expires_at)}` : data.sync.service_session.state === "owner_managed" ? "Secret local privé, absent de la base et du dépôt" : "Aucun mot de passe IMT n'est conservé par IMTégrale"}</small></div>
          <button className="secondary-button" type="button" onClick={() => setPassReconnectOpen(true)}><RefreshCw size={16} /> {data.sync.service_session.reauth_required ? "Reconnecter" : "Renouveler"}</button>
        </div>
        {data.sync.service_session.hub_state === "degraded" && <div className="settings-hint"><Info size={14} /> PASS fonctionne, mais HUB COMPETENCES devra peut-être être rouvert lors de la prochaine reconnexion.</div>}
      </section>
      ...
      <PassReconnectModal open={passReconnectOpen} identifier={data.account.imt_username} onClose={() => setPassReconnectOpen(false)} onRenewed={refresh} />
    </div>
  );
}
