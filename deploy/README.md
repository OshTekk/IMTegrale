...
- Vérifier qu'un nouveau participant est visible par un participant actif dès son inscription, mais ne reçoit lui-même ni lignes ni compteur avant l'échéance de 48 heures. Ses coefficients ECTS doivent avoir été verrouillés automatiquement. Après la migration `0009`, un ancien participant aux ECTS incomplets doit être replacé en attente d'activation sans délai de réinscription.
- Vérifier qu'une synchronisation avec métadonnées échues importe depuis COMPETENCES les intitulés et crédits alloués, marque leur source officielle et empêche leur modification par l'étudiant. La migration `0010` demande ce rafraîchissement à tous les comptes sans l'exécuter elle-même.
